...
::===============================================================================================================
cls
call :MenuHeader "[HEADER] VC REDIST 2015 运行库检查"
echo:
echo [ INFO ] 未安装VC REDIST 2015 运行库. 
echo:
echo [ INFO ] 安装 将下载burfadels主要数据程序进行安装(推荐).
echo:
echo [ INFO ] 跳过 将下载独立的DLL到PHP文件夹.
call :Footer
echo:
CHOICE /C IS /N /M "[ 选项 ]  现在安装[I]  跳过[S] :"
...
::===============================================================================================================
cls
call :MenuHeader "[HEADER] 主菜单 [UUP DUMP STANDALONE] [VCREDIST : %vcr% : %vcarch% on %vera%]"
call :MenuFooter
echo:
...
call :Footer
CHOICE /C CS /N /M "[ 选项 ]  开始下载[C]  跳过[S] :"
...
for /f "tokens=1,2,3,4 delims=|" %%a in ('type "_temp\checkids.txt" ^| findstr /v /c:"%uupid%" ^| findstr /c:"%arch%" ^| findstr /c:"%buildstr%"') do (
	set "check=1"
	powershell "Write-Host '[OLD ID] %%c | 删除'  -foreground DarkMagenta"
...
call :Footer
CHOICE /C DNB /N /M "[ 选项 ]  下载[D]  重新搜索[N]  返回[B] :"
if %errorlevel%==2 goto:UUPQueryLatest
if %errorlevel%==3 goto:M50DLMainMenu
call :Footer
call :LangChoice14
call :Footer
CHOICE /C AE /N /M "[ 选项 ]  多版合一[A]   版本选择[E] :"
...
:UUPQueryOlder
cls
call :MenuHeader "[HEADER] 查询本地数据库"
call :MenuFooter
call :UUPDLListDatabase
call :MenuFooter
call :MenuFooter
echo:
echo [ USER ] 输入版本号 如: 16353.1000, 16251.0,... 
echo [ INFO ] 默认值: %buildstr%
echo:
call :MenuFooter
call :MenuFooter
echo:
set /p buildstr=[ 选项 ] 输入版本号后按确认键: 
...
if exist "uup\%downfolder%" (
	call :Header "[HEADER] 文件已存在"
	powershell "Write-Host '[ 提示 ] 下载文件已经存在.' -foreground DarkMagenta"
	call :Footer
	CHOICE /C PS /N /M "[ 选项 ]  继续[P]  跳过[S] :"
	if !errorlevel!==2 goto:TempDeleteLatest
)
cls
call :MenuHeader "[HEADER] UUP链接查询"
call :MenuFooter
echo:
echo [ INFO ] 下载列表 %sku%.
call :Footer
...
:UUPAiOWarn
if "%sku%"=="AiO" if "%x%"=="0" (
	powershell "Write-Host '[ 提示 ] 没有发现ESD版本.'  -foreground DarkMagenta"
	echo:
	call :MenuFooter
	call :MenuFooter
	echo:
...
if "%abbodipresent%"=="yes" (
	call :Header "[ 确认全部校验OK再进行转换 否则重新下载 ]  UUP转WIN/ESD/ISO"
	CHOICE /C YN /N /M "[ 选项 ] 现在开始 [Y/N] :"
	if !errorlevel!==1 goto:AbbodiStartLatest
	call :Footer
)
CHOICE /C YN /N /M "删除临时文件夹 [Y/N] :"
...
::===============================================================================================================
cls
call :MenuHeader "[HEADER] RTM累积更新下载"
call :MenuFooter
echo:
CHOICE /C 68A /N /M "[ 选项 ]  X64[6]  X86[8]  Arm64[A] :"
if %errorlevel%==1 set "arch=amd64"
...
if not exist "_temp" md "_temp"
cls
call :MenuHeader "[HEADER] 检查Windows内部所有通道"
call :MenuFooter
call %listid%>>"_temp\checkids.txt"
call :MenuHeader "[HEADER] 检查Windows内部快速跳跃通道"
call :WIFS
echo:
call :MenuHeader "[HEADER] 检查Windows内部快速激活通道"
call :WIFA
echo:
call :MenuHeader "[HEADER] 检查Windows内部慢速激活通道"
call :WISA
echo:
call :MenuHeader "[HEADER] 检查Windows预览释放通道"
call :WRPA
echo:
call :MenuFooter
call :MenuFooter
echo:
CHOICE /C NB /N /M "[ 选项 ]  重新搜索[N]  返回[B] :"
...
::===============================================================================================================
cls
call :MenuHeader "[HEADER] UUP信息查询"
call :MenuFooter
call :UUPDLListDatabase
call :MenuFooter
call :MenuFooter
...
if %errorlevel%==3 set "arch=arm64"
call :Footer
CHOICE /C SMA /N /M "[ 选项 ]  单语言[S]  多语言[M]  所有语言[A] :"
if %errorlevel%==2 (
	call :Footer
	echo [ USER ] 语言列表 如: zh-cn, zh-tw, en-us...
	echo [ INFO ] 默认值: %lang%
	echo:
	call :Footer
	set /p lang=[ 选项 ] 输入语言后按确认键 :  
	goto:QuerySpecific
)
if %errorlevel%==3 (
...
::===============================================================================================================
:NothingToDoSpecific
powershell "Write-Host '[ 提示 ] 这里没有什么可做.' -foreground DarkMagenta"
call :Footer
echo [ 选项 ] 按任意键返回 ^>^>
pause >nul 2>&1
if exist "_temp" rd /s /q "_temp" >nul 2>&1
...
pushd %~dp0
cls
call :MenuHeader "[HEADER] 数据库更新检查"
call :MenuFooter
echo:
echo [ INFO ] 正在检查...
...
if not exist "fileinfo\compare.txt" (
	...
	call :MenuFooter
	echo:
	powershell "Write-Host '[ 提示 ] 没有找到新的构建信息.' -foreground DarkMagenta"
	echo:
	call :MenuFooter
	call :MenuFooter
	echo:
...
powershell "Write-Host '[ 提示 ] 检测到新的UUP IDs.' -foreground Cyan"
echo [ INFO ] 下载链接信息.
call :Footer
for /f "tokens=1,2,3,4 delims=|" %%a in ('type "fileinfo\compare.txt"') do (
	...
	powershell "Write-Host '[ 提示 ] %%c | 下载'  -foreground Cyan"
	call :Footer
)
call :Footer
echo [ INFO ] 检查已取代的IDs.
...
if %projectver% geq %projectvernew% (
	echo [ INFO ] 当前版本: %projectver%
	echo [ INFO ] 检查版本: %projectvernew%
	call :Footer
	powershell "Write-Host '[ 提示 ] 没有发现更新.' -foreground DarkMagenta"
	echo:
	call :MenuFooter
	call :MenuFooter
	echo:
	CHOICE /C DS /N /M "[ 选项 ]   下载[D]   跳过[S] :"
	if !errorlevel!==1 goto:ProjectDownload
	if !errorlevel!==2 if "%auto%"=="1" goto:VCRedistInstallCheck
	if !errorlevel!==2 goto:M50DLMainMenu
)
powershell "Write-Host '[ 提示 ] 新版本: %projectvernew%' -foreground Cyan"
call :Footer
echo:
call :MenuFooter
call :MenuFooter
...
::===============================================================================================================
::===============================================================================================================
CHOICE /C RS /N /M "[ 选项 ]  替换[R]  跳过[S] :"
if %errorlevel%==2 if "%auto%"=="1" goto:VCRedistInstallCheck
if %errorlevel%==2 goto:M50DLMainMenu
call :UUPDLUpdateScript
call :FOOTER
echo [ INFO ] 现在开始更新 工具将关闭.
call :FOOTER
echo [ 选项 ] 按任意键返回 ^>^>
pause >nul 2>&1
...
for /f "tokens=1,2,3,4 delims=|" %%a in ('type "%~1" ^| findstr /v /c:"%uupid%" ^| findstr /c:"%arch%" ^| findstr /c:"%buildstr%"') do (
	set "check=1"
	if [%%c] equ [] powershell "Write-Host '[OLD ID] %%c | 删除'  -foreground DarkMagenta"
	powershell "Write-Host '[OLD ID] %%c | 删除'  -foreground DarkMagenta"
...
:NonAiOCheck
	powershell "Write-Host '[ 提示 ] %~1 | 没有发现ESD版本.'  -foreground DarkMagenta"
	echo:
	set "proceed=0"
	call :MenuFooter
	call :MenuFooter
...
:: EDITION CHOICE
:SKUChoice
call :Header "[HEADER] 版本选择"
echo:
echo [  A   ] Core [核心版]
echo [  B   ] CoreN [核心N版]
echo:
echo [  C   ] CoreSingleLanguage [单语言核心版]
echo [  D   ] CoreCountrySpecific [核心特异版]
echo:
echo [  E   ] Cloud [云端版]
echo [  F   ] CloudN [云端N版]
echo:
echo [  G   ] Professional [专业版]
echo [  H   ] ProfessionalN [专业N版]
echo: 
echo [  I   ] Enterprise [企业版]
echo [  J   ] EnterpriseN [企业N版]
echo:
echo [  K   ] Education [教育版]
echo [  L   ] EducationN [教育N版]
echo:
call :MenuFooter
echo:
CHOICE /C ABCDEFGHIJKL /N /M "[ 选项 ] 请选择 :"
...
call :MenuFooter
echo:
CHOICE /C 1234567890 /N /M "[ 选项 ] 输入第2位数字 :"
